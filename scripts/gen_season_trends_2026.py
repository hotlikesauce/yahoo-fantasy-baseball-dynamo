...
# Categories - TB is pitching Total Bases Allowed (lower = better)
...
# ============================================================
# 1. Build team mapping from power_ranks_live (2026 Lambda data)
# ============================================================
name_to_tn = {}
tn_latest_name = {}
power_data = {}
...
# Also build mapping from weekly_stats (in case power_ranks_live isn't available)
...
if not stat_weeks:
    print("No weekly stats data found. Run pull_weekly_stats Lambda first.")
    sys.exit(1)
...
# Sort by final power score if available, otherwise by cumulative xWins
if power_weeks:
    ...
    sorted_teams = sorted(cum_xw.items(), key=lambda x: x[1], reverse=True)
    # Convert to same format as power_data items for compatibility
    sorted_teams = [(tn, power_data.get(tn, {})) for tn, _ in sorted_teams]

color_map = {tn: colors[i % len(colors)] for i, (tn, _) in enumerate(sorted_teams)}

def js_label(tn):
    return tn_latest_name.get(tn, tn).replace('\\', '\\\\').replace("'", "\\'")

# Power Score/Rank datasets (only if we have power data)
has_power = len(power_weeks) > 0
...
<p class="section-desc">The single best (or lowest for ERA/WHIP/TB) team stat line in any week this season across all 12 scoring categories.</p>
<div style="margin-bottom: 12px; display: flex; gap: 8px; flex-wrap: wrap;">
  <button class="week-filter-btn active" data-week-type="all" onclick="filterWeeks('all')">All Weeks</button>
  <button class="week-filter-btn" data-week-type="short" onclick="filterWeeks('short')">Short Weeks</button>
...
with open(r'c:\Users\taylor.ward\Documents\yahoo-fantasy-baseball-dynamo\docs\season_trends_2026.html', 'w', encoding='utf-8') as f:
    f.write(html)

print(f'\nGenerated docs/season_trends_2026.html')
print(f'{len(sorted_teams)} teams, stat weeks: {stat_weeks}')
for i, (tn, data) in enumerate(sorted_teams):
    bat = season_bat[tn]
    pit = season_pit[tn]
    bpct = round(bat['w'] / bat['total'] * 100, 1) if bat['total'] > 0 else 0
    ppct = round(pit['w'] / pit['total'] * 100, 1) if pit['total'] > 0 else 0
    print(f'  {colors[i % len(colors)]} {tn_latest_name.get(tn, tn)} (Bat:{bpct}% Pit:{ppct}%)')
