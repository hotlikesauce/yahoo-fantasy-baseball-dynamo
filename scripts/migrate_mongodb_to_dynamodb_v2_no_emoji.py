...
def write_to_season_trends(collection_name: str, df: pd.DataFrame, dynamodb):
    ...
    if not team_field:
        print(f"   [WARNING]  No team number field found, skipping")
        return 0
    ...
    for idx, row in df.iterrows():
        week_num = int(row[week_field]) if has_week and pd.notna(row.get(week_field)) else 0
        team_num = str(row[team_field])

        # Create composite sort key: DataType#Week
        sort_key = f"{collection_name}#{week_num}"
...
def main():
    ...
    print("\n" + "="*70)
    print("📦 MongoDB → DynamoDB Migration (v2 - 3 Table Design)")
    print("="*70)
